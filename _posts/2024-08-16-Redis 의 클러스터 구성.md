...
redis.conf 에서 클러스터를 생성하기 위해 제가 커스텀한 설정은 `daemonize`, `port`, `cluster-enabled`, `cluster-node-timeout`, `maxmemory`, `dir` 입니다. 저는 6가지 설정을 아래와 같이 path/to/redis.conf 파일 상에서 편집하여 모든 Redis 노드가 동일한 설정으로 부여하였습니다. (참고로 정상적인 Replication 을 위해선 maxmemory 옵션은 꼭 동일하여야 합니다.)
